...
#public Subnet 3
resource "aws_subnet" "elearning_vpc_public_subnet_3" {
  vpc_id     = aws_vpc.elearning_vpc.id
  cidr_block = var.ELEARNING_VPC_PUBLIC_SUBNET3_CIDR_BLOCK
  availability_zone = data.aws_availability_zones.available.names[2]
  map_public_ip_on_launch = "true"
  tags = {
    Name = "${var.ENVIRONMENT}-elearning-vpc-public-subnet-3"
  }
}
...
# private subnet 2
resource "aws_subnet" "elearning_vpc_private_subnet_3" {
  vpc_id     = aws_vpc.elearning_vpc.id
  cidr_block = var.ELEARNING_VPC_PRIVATE_SUBNET3_CIDR_BLOCK
  availability_zone = data.aws_availability_zones.available.names[2]
  tags = {
    Name = "${var.ENVIRONMENT}-elearning-vpc-private-subnet-3"
  }
}
...
resource "aws_route_table_association" "to_public_subnet3" {
  subnet_id      = aws_subnet.elearning_vpc_public_subnet_3.id
  route_table_id = aws_route_table.public.id
}
...
resource "aws_route_table_association" "to_private_subnet3" {
  subnet_id      = aws_subnet.elearning_vpc_private_subnet_3.id
  route_table_id = aws_route_table.private.id
}
...
output "private_subnet3_id" {
  description = "Subnet ID"
  value       = aws_subnet.elearning_vpc_private_subnet_3.id
}
